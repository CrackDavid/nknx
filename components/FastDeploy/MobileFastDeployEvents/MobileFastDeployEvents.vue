<template>
  <Card col="12" padding="none">
    ...
    <div class="mobile-fd-event">
      <div v-for="event in events" :key="event.id" class="mobile-fd-event__item">
        <div class="mobile-fd-event__title">{{ $t('deploymentName') }}</div>
        <div class="mobile-fd-event__value text_wrap_none">{{ event.fd_deployment.label }}</div>
        <div class="mobile-fd-event__title">{{ $t('server') }}</div>
        <div class="mobile-fd-event__value text_wrap_none">{{ event.fd_deployment.ip }}</div>
        <div class="mobile-fd-event__title">{{ $t('provider') }}</div>
        <div class="mobile-fd-event__value text_wrap_none">{{ event.fd_deployment.provider }}</div>
        <div class="mobile-fd-event__title">{{ $t('event') }}</div>
        <div class="mobile-fd-event__value text_wrap_none">{{ $t(`fdEvent${event.event_code}`) }}</div>
        <div class="mobile-fd-event__title">{{ $t('when') }}</div>
        <div
          class="mobile-fd-event__value text_wrap_none"
        >{{ $moment(event.created_at).format('MMM Do, hh:mm:ss a') }}</div>
      </div>
    </div>
  </Card>
</template>
